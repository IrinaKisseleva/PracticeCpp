...
public:
	...
	void workDay() {
		...
		for (int time = 0; time < working_minutes; time++) {
			if (time % 10 == 0) {
				total_visitors += generateVisitors(randomInt(0, 3), time);
			}

			if ((time == visitor_change_time && ((time <= start_lunch) or (time > end_lunch))) or ((visitor_change_time < time) && (time = end_lunch))) {
				total_managed++;
				if (time != 0) {
